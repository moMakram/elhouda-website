...
<table width="700" border="0" style="text-align:center; margin-left:50px; margin-top:40px;">
  <tr style="background:#999; height:35px;">
    ...
    <td><font face="Comic Sans MS, cursive"  color="#000000">قسم الفيديو</font></td>
	<td><font face="Comic Sans MS, cursive"  color="#000000">الرابط</font></td>
  ...
  ');
	 $sel = mysql_query("select * from viditem ");
			  
			while($row = mysql_fetch_array($sel)){
			
			echo('
  <tr style="background:#03C;height:30px;">
    <td>'); echo $row['vidsec']; echo('</td>
	<td>'); echo $row['descr']; echo('</td>
	 <td>'); echo $row['ordere']; echo('</td>
    
    

	<td>');  echo (' <a href="delete_video2.php?id= '); echo $row['id']; echo('"> <font color="white"> delete </font></a></td>
  </tr> '); } 
